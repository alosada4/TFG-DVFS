...
void sendBeats(FILE* fp, long int starting_line)
{
	...
	for(file_line = 0; file_line < starting_line; file_line++)
	{
		int n = fscanf(fp, "%d", &X);
	...
	fprintf(stderr, "End of file. Finished.\n");
	timespec_get(&tval, TIME_UTC);
...
int main(int argc, char* argv[])
{
	float result;
	int I, J;
	long int starting_line;


	if(argc < 5)
	{
		fprintf(stderr,"Usage: %s <moving-average-order> <filter-coeffs-file>  <single-beat-data-file> <tty> [starting-line]\n", 
				argv[0]);
		return(1);
	}
	else if (argc < 6)
	...
	// send the ecg data
	sendBeats(beats_file_pointer, starting_line);
